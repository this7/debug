<style type="text/css">
@media (min-width:768px) {
    body[data-v-33fe74b2] {
        padding-bottom: 42px
    }
}

@media (max-width:767px) {
    #debug[data-v-33fe74b2] {
        display: none
    }
}

#debug abbr[title][data-v-33fe74b2] {
    border-bottom: 1px dotted
}

#debug b[data-v-33fe74b2],
#debug strong[data-v-33fe74b2] {
    font-weight: 700
}

#debug dfn[data-v-33fe74b2] {
    font-style: italic
}

#debug hr[data-v-33fe74b2] {
    box-sizing: content-box;
    height: 0
}

#debug code[data-v-33fe74b2],
#debug kbd[data-v-33fe74b2],
#debug pre[data-v-33fe74b2],
#debug samp[data-v-33fe74b2] {
    font-family: Consolas, monospace, serif;
    font-size: 1em
}

#debug pre[data-v-33fe74b2] {
    white-space: pre-wrap
}

#debug q[data-v-33fe74b2] {
    quotes: "\201C" "\201D" "\2018" "\2019"
}

#debug small[data-v-33fe74b2] {
    font-size: 80%
}

#debug table[data-v-33fe74b2] {
    border-collapse: collapse;
    border-spacing: 0
}

#debug[data-v-33fe74b2] {
    font-family: Helvetica Neue, Helvetica, Arial, sans-serif;
    font-size: 12px;
    font-weight: 400;
    line-height: 20px;
    display: none;
}

#debug a[data-v-33fe74b2] {
    text-decoration: none
}

#debug a[data-v-33fe74b2]:hover {
    text-decoration: underline
}

#debug a[data-v-33fe74b2] {
    color: #37a5e5
}

#debug a[data-v-33fe74b2]:hover {
    color: #1889c7
}

#debug em[data-v-33fe74b2] {
    color: #f0615c
}

#debug ins[data-v-33fe74b2] {
    background: #ffd;
    color: #444;
    text-decoration: none
}

#debug mark[data-v-33fe74b2] {
    background: #ffd;
    color: #444
}

#debug[data-v-33fe74b2] ::-moz-selection {
    background: #39f;
    color: #fff;
    text-shadow: none
}

#debug[data-v-33fe74b2] ::selection {
    background: #39f;
    color: #fff;
    text-shadow: none
}

#debug abbr[title][data-v-33fe74b2],
#debug dfn[title][data-v-33fe74b2] {
    cursor: help
}

#debug dfn[title][data-v-33fe74b2] {
    border-bottom: 1px dotted;
    font-style: normal
}

#debug address[data-v-33fe74b2],
#debug blockquote[data-v-33fe74b2],
#debug dl[data-v-33fe74b2],
#debug fieldset[data-v-33fe74b2],
#debug figure[data-v-33fe74b2],
#debug hr[data-v-33fe74b2],
#debug ol[data-v-33fe74b2],
#debug p[data-v-33fe74b2],
#debug pre[data-v-33fe74b2],
#debug ul[data-v-33fe74b2] {
    margin: 0 0 15px
}

#debug *+address[data-v-33fe74b2],
#debug *+blockquote[data-v-33fe74b2],
#debug *+dl[data-v-33fe74b2],
#debug *+fieldset[data-v-33fe74b2],
#debug *+figure[data-v-33fe74b2],
#debug *+hr[data-v-33fe74b2],
#debug *+ol[data-v-33fe74b2],
#debug *+p[data-v-33fe74b2],
#debug *+pre[data-v-33fe74b2],
#debug *+ul[data-v-33fe74b2] {
    margin-top: 15px
}

#debug h1[data-v-33fe74b2],
#debug h2[data-v-33fe74b2],
#debug h3[data-v-33fe74b2],
#debug h4[data-v-33fe74b2],
#debug h5[data-v-33fe74b2],
#debug h6[data-v-33fe74b2] {
    margin: 0 0 15px;
    font-family: Helvetica Neue, Helvetica, Arial, sans-serif;
    font-weight: 400;
    color: #444;
    text-transform: none
}

#debug *+h1[data-v-33fe74b2],
#debug *+h2[data-v-33fe74b2],
#debug *+h3[data-v-33fe74b2],
#debug *+h4[data-v-33fe74b2],
#debug *+h5[data-v-33fe74b2],
#debug *+h6[data-v-33fe74b2] {
    margin-top: 25px
}

#debug .box-h1[data-v-33fe74b2],
#debug h1[data-v-33fe74b2] {
    font-size: 24px;
    line-height: 30px
}

#debug .box-h2[data-v-33fe74b2],
#debug h2[data-v-33fe74b2] {
    font-size: 18px;
    line-height: 24px
}

#debug .box-h3[data-v-33fe74b2],
#debug h3[data-v-33fe74b2] {
    font-size: 16px;
    line-height: 22px
}

#debug .box-h4[data-v-33fe74b2],
#debug h4[data-v-33fe74b2] {
    font-size: 14px;
    line-height: 20px
}

#debug .box-h5[data-v-33fe74b2],
#debug .box-h6[data-v-33fe74b2],
#debug h5[data-v-33fe74b2],
#debug h6[data-v-33fe74b2] {
    font-size: 12px;
    line-height: 18px
}

#debug ol[data-v-33fe74b2],
#debug ul[data-v-33fe74b2] {
    padding-left: 30px
}

#debug ol>li>ol[data-v-33fe74b2],
#debug ol>li>ul[data-v-33fe74b2],
#debug ul>li>ol[data-v-33fe74b2],
#debug ul>li>ul[data-v-33fe74b2] {
    margin: 0
}

#debug dt[data-v-33fe74b2] {
    font-weight: 700
}

#debug dd[data-v-33fe74b2] {
    margin-left: 0
}

#debug hr[data-v-33fe74b2] {
    display: block;
    padding: 0;
    border: 0;
    border-top: 1px solid #e5e5e5
}

#debug code[data-v-33fe74b2] {
    padding: 0 4px;
    border: 1px solid #e5e5e5;
    border-radius: 3px;
    background: #fafafa;
    color: #f0615c;
    white-space: nowrap;
    font-size: 12px
}

#debug pre code[data-v-33fe74b2] {
    padding: 0;
    border: 0;
    background: transparent;
    color: inherit;
    white-space: pre-wrap
}

#debug pre[data-v-33fe74b2] {
    padding: 10px;
    border: 1px solid #e5e5e5;
    border-radius: 3px;
    background: #fafafa;
    color: #444;
    -moz-tab-size: 4;
    -o-tab-size: 4;
    tab-size: 4;
    line-height: 18px;
    font-size: 12px
}

#debug[data-v-33fe74b2] {
    position: fixed;
    width: 100%;
    height: 370px;
    bottom: 0;
    padding: 0;
    margin: 0;
    z-index: 999;
}

#debug .navbar[data-v-33fe74b2],
#debug[data-v-33fe74b2] {
    left: 0;
    border-top: 1px solid rgba(0, 0, 0, .06);
    border-bottom: 1px solid rgba(0, 0, 0, .11);
    background: #fff linear-gradient(180deg, #fff, #f7f7f7);
    color: #444
}

#debug .navbar[data-v-33fe74b2] {
    position: absolute;
    top: 0;
    height: 42px;
    width: 100%;
}

#debug .navbar>ul[data-v-33fe74b2] {
    margin: 0;
    padding: 0;
    float: left;
    list-style: none
}

#debug .navbar>ul>li[data-v-33fe74b2] {
    position: relative;
    float: left;
    display: list-item;
    text-align: -webkit-match-parent;
    margin: 0;
    padding: 0;
}

#debug .navbar>ul>li>a[data-v-33fe74b2] {
    display: block;
    box-sizing: border-box;
    height: 41px;
    margin-top: -1px;
    padding: 0 30px;
    border-top: 1px solid transparent;
    border-right: 1px solid rgba(0, 0, 0, .11);
    color: #444;
    text-decoration: none;
    line-height: 40px;
    font-size: 12px;
}

#debug .navbar>ul>li>a.active[data-v-33fe74b2] {
    margin-top: 0px;
    background: #fff
}

#debug div.box[data-v-33fe74b2] {
    position: absolute;
    left: 0;
    top: 43px;
    width: 100%;
    height: 327px;
    padding: 20px;
    box-sizing: border-box;
    overflow-x: hidden;
    overflow-y: auto
}

#debug .hide[data-v-33fe74b2] {
    display: none
}

#debug div .active[data-v-33fe74b2] {
    display: block!important
}

.icon[data-v-33fe74b2] {
    width: 16px;
    height: 16px
}

.icon-large[data-v-33fe74b2],
.icon[data-v-33fe74b2] {
    display: inline-block;
    background-repeat: no-repeat;
    vertical-align: middle
}

.icon-large[data-v-33fe74b2] {
    width: 34px;
    height: 26px;
    margin: 0 -5px
}

.icon-pagekit[data-v-33fe74b2] {
    background-image: url(data:image/svg+xml;base64,)
}

.icon-request[data-v-33fe74b2] {
    background-image: url(data:image/svg+xml;base64,)
}




.icon-logo[data-v-33fe74b2] {
    background-image: url(data:image/svg+xml;base64,)
}

.icon-time[data-v-33fe74b2] {
    background-image: url(data:image/svg+xml;base64,)
}

.icon-memory[data-v-33fe74b2] {
    background-image: url(data:image/svg+xml;base64,)
}

.icon-events[data-v-33fe74b2] {
    background-image: url(data:image/svg+xml;base64,)
}

.icon-database[data-v-33fe74b2] {
    background-image: url(data:image/svg+xml;base64,)
}

.icon-auth[data-v-33fe74b2] {
    background-image: url(data:image/svg+xml;base64,)
}

.icon-routes[data-v-33fe74b2] {
    background-image: url(data:image/svg+xml;base64,)
}

.icon-events[data-v-33fe74b2],
.icon-request[data-v-33fe74b2] {
    width: 20px
}

#debug .close[data-v-33fe74b2] {
    display: inline-block;
    position: absolute;
    top: 10px;
    right: 10px;
    width: 20px;
    color: inherit;
    text-align: center;
    line-height: 20px;
    opacity: .3
}

#debug .close[data-v-33fe74b2]:after {
    content: "\D7";
    display: block;
    font-family: FontAwesome
}

#debug .box-table[data-v-33fe74b2] {
    width: 100%;
    margin-bottom: 15px
}

#debug .submenu[data-v-33fe74b2] {
    padding-left: 5px;
}

#debug *+.box-table[data-v-33fe74b2] {
    margin-top: 15px
}

#debug .box-table td[data-v-33fe74b2],
#debug .box-table th[data-v-33fe74b2] {
    padding: 8px;
    border-bottom: 1px solid #e5e5e5
}

#debug .box-table th[data-v-33fe74b2] {
    text-align: left
}

#debug .box-table td[data-v-33fe74b2] {
    vertical-align: top
}

#debug .box-table thead th[data-v-33fe74b2] {
    vertical-align: bottom
}

#debug .box-table caption[data-v-33fe74b2],
#debug .box-table tfoot[data-v-33fe74b2] {
    font-style: italic;
    font-size: 12px
}

#debug .box-table caption[data-v-33fe74b2] {
    color: #999;
    text-align: left
}

#debug .box-table td[data-v-33fe74b2]:first-child {
    width: 300px
}

#debug .box-table tbody tr:hover td[data-v-33fe74b2] {
    background: #ffd
}

#debug .box-table-dropdown td[data-v-33fe74b2]:first-child {
    font-weight: 700
}

#debug .box-table-dropdown tbody tr:last-child td[data-v-33fe74b2] {
    border-bottom: none
}

#debug .box-table tr.box-active[data-v-33fe74b2] {
    background: #eee
}
#this7debug_logo[data-v-33fe74b2]{
    position:fixed;
    right: 0;
    top: 50%;
    width: 30px;
    height: 30px;
    background-color:#6ee2d8;
    border-top-left-radius: 10px;
    -moz-border-top-left-radius: 10px;
    -webkit-border-top-left-radius: 10px;
    border-bottom-right-radius: 10px;
    -moz-border-bottom-right-radius: 10px;
    -webkit-border-bottom-right-radius: 10px;
    color: #fff;
    opacity:0.4;
    display: block;
}
</style>
<div data-v-33fe74b2 id="this7debug_logo" class="icon icon-logo"></div>
<div data-v-33fe74b2 id="debug">
    <div data-v-33fe74b2 class="navbar">
        <ul data-v-33fe74b2 id="menu">
            <li data-v-33fe74b2><a data-v-33fe74b2 class="active"><span data-v-33fe74b2 class="icon icon-events"></span> 提示</a></li>
            <li data-v-33fe74b2><a data-v-33fe74b2><span data-v-33fe74b2 class="icon icon-routes"></span> 加载</a></li>
            <li data-v-33fe74b2><a data-v-33fe74b2><span data-v-33fe74b2 class="icon icon-database"></span> 数据</a></li>
            <li data-v-33fe74b2><a data-v-33fe74b2><span data-v-33fe74b2 class="icon icon-time"></span> 运行</a></li>
            <li data-v-33fe74b2><a data-v-33fe74b2><span data-v-33fe74b2 class="icon icon-memory"></span> 系统</a></li>
        </ul>
        <a data-v-33fe74b2 id="this7debug_close" class="close"></a>
    </div>
    <div data-v-33fe74b2 class="box active">
        <h3>页面提示信息</h3>
    </div>
    <div data-v-33fe74b2 class="box hide">
        <table data-v-33fe74b2 class="box-table">
            <thead data-v-33fe74b2>
                <tr data-v-33fe74b2>
                    <th data-v-33fe74b2>IP</th>
                    <th data-v-33fe74b2>Method</th>
                    <th data-v-33fe74b2>URL</th>
                    <th data-v-33fe74b2>Time</th>
                    <th data-v-33fe74b2>Request Id</th>
                </tr>
            </thead>
            <tbody data-v-33fe74b2>
                <?php if (!empty($loading) && is_array($loading)): ?>
                <?php foreach ($loading as $key => $value): ?>
                <?php if ($key === 0): ?>
                <tr data-v-33fe74b2 class="box-active">
                    <?php else: ?>
                    <tr data-v-33fe74b2>
                        <?php endif;?>
                        <td data-v-33fe74b2>
                            <?php echo $value['name']; ?>
                        </td>
                        <td data-v-33fe74b2>
                            <?php echo $value['mold']; ?>
                        </td>
                        <td data-v-33fe74b2>
                            <?php echo $value['file']; ?>
                        </td>
                        <td data-v-33fe74b2>
                            <?php echo $value['time']; ?>
                        </td>
                        <td data-v-33fe74b2>
                            <a data-v-33fe74b2>
                                <?php echo $value['id']; ?>
                            </a>
                        </td>
                    </tr>
                    <?php endforeach;?>

                <?php endif;?>
            </tbody>
        </table>
    </div>
    <div data-v-33fe74b2 class="box hide">
        <?php foreach (sql::getQueryLog() as $key => $value): ?>
        <div data-v-33fe74b2>
            <pre data-v-33fe74b2><code data-v-33fe74b2 ><?php echo $value['sql']; ?></code></pre>
            <p class="submenu" data-v-33fe74b2>
                <span data-v-33fe74b2><?php echo $value['time']; ?> </span>
                <span data-v-33fe74b2><?php echo mb_substr($value['data'], 0, 60, "UTF8") . "..."; ?></span>
            </p>
        </div>
        <?php endforeach;?>
    </div>
    <div data-v-33fe74b2 class="box hide">
            <h1 data-v-33fe74b2>运行信息</h1>
            <h2 data-v-33fe74b2>系统</h2>
            <table data-v-33fe74b2 class="box-table box-table-dropdown">
                <tbody data-v-33fe74b2>
                    <tr data-v-33fe74b2>
                        <td data-v-33fe74b2>This7框架</td>
                        <td data-v-33fe74b2><?php echo VERSION; ?></td>
                    </tr data-v-33fe74b2>
                    <tr data-v-33fe74b2>
                        <td data-v-33fe74b2>服务器</td>
                        <td data-v-33fe74b2><?php echo $_SERVER['SERVER_SOFTWARE']; ?></td>
                    </tr data-v-33fe74b2>
                    <tr data-v-33fe74b2>
                        <td data-v-33fe74b2>浏览器</td>
                        <td data-v-33fe74b2><?php echo $_SERVER['HTTP_USER_AGENT']; ?></td>
                    </tr>
                </tbody>
            </table>
            <h2 data-v-33fe74b2>PHP</h2>
            <table class="box-table box-table-dropdown">
                <tbody data-v-33fe74b2>
                    <tr data-v-33fe74b2>
                        <td data-v-33fe74b2>PHP</td>
                        <td data-v-33fe74b2><?php echo PHP_VERSION; ?></td>
                    </tr>
                    <tr data-v-33fe74b2>
                        <td data-v-33fe74b2>PHP SAPI</td>
                        <td data-v-33fe74b2><?php echo php_sapi_name(); ?></td>
                    </tr>
                    <tr data-v-33fe74b2>
                        <td data-v-33fe74b2>System</td>
                        <td data-v-33fe74b2><?php echo php_uname(); ?></td>
                    </tr>
                    <tr data-v-33fe74b2>
                        <td data-v-33fe74b2>Extensions</td>
                        <td data-v-33fe74b2><?php echo implode(", ", get_loaded_extensions()); ?></td>
                    </tr>
                </tbody>
            </table>
            <h2 data-v-33fe74b2>数据库</h2>
            <table class="box-table box-table-dropdown" data-v-33fe74b2>
                <tbody data-v-33fe74b2>
                    <tr data-v-33fe74b2>
                        <td data-v-33fe74b2>驱动</td>
                        <td data-v-33fe74b2><?php echo sql::getDriveInfo()['name']; ?></td>
                    </tr>
                    <tr data-v-33fe74b2>
                        <td data-v-33fe74b2>版本</td>
                        <td data-v-33fe74b2><?php echo sql::getDriveInfo()['version']; ?></td>
                    </tr>
                    <tr data-v-33fe74b2>
                        <td data-v-33fe74b2>客户端</td>
                        <td data-v-33fe74b2><?php echo sql::getDriveInfo()['client']; ?></td>
                    </tr>
                </tbody>
            </table>

    </div>
    <div data-v-33fe74b2 class="box hide">
        <h1>系统常量</h1>
        <?php foreach (get_defined_constants(true)['user'] as $key => $value): ?>
        <pre data-v-33fe74b2><code data-v-33fe74b2 >[<?php echo $key; ?>]:<?php echo $value; ?></code></pre>
        <?php endforeach;?>
    </div>
</div>
<script type="text/javascript">
// JS实现选项卡切换
window.onload = function() {
    //获取debug下的所有div
    var debug = document.getElementById("debug");
    var this7debug_logo = document.getElementById("this7debug_logo");
    var this7debug_close = document.getElementById("this7debug_close");
    var debug_divs = debug.getElementsByClassName("box");

    //获取menu下的所有li
    var debug_ul = document.getElementById("menu");
    var debug_lis = debug_ul.getElementsByTagName("a");

    //循环遍历信息
    for (var i = 0, len = debug_lis.length; i < len; i++) {
        debug_lis[i].index = i;
        debug_lis[i].onclick = function() {
            for (var j = 0; j < len; j++) {
                debug_lis[j].classList.remove("active");
                debug_divs[j].classList.remove("active");
                debug_divs[j].classList.remove("hide");
                debug_divs[j].classList.add("hide");
            }
            this.classList.add("active");
            debug_divs[this.index].classList.remove("hide");
            debug_divs[this.index].classList.add("active");
        }
    }

    this7debug_logo.onclick = function(){
        debug.style.display= "block";
        this7debug_logo.style.display= "none";
    }
    this7debug_close.onclick = function(){
        debug.style.display= "none";
        this7debug_logo.style.display= "block";
    }
}
</script>